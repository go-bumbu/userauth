...
func testServer(sessionDur, maxSessionDur, update time.Duration, allowRenew bool, storeType string) (*httptest.Server, *http.Client) {
	...
	auth := &userauth.LoginHandler{UserStore: dummyUser{}}
	pendingStore := pendingmemory.New()
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/json-login":
			loginhandler.JsonAuthHandler(authSess, auth, pendingStore, 5*time.Minute).ServeHTTP(w, r)
		default:
