// Package memory provides an in-memory loginflow.AttemptStore.
package memory
...
type Store struct {
	mu    sync.Mutex
	store map[string]loginflow.Attempt
}

func New() *Store {
	return &Store{
		store: make(map[string]loginflow.Attempt),
	}
}

func (m *Store) Set(_ *http.Request, _ http.ResponseWriter, a loginflow.Attempt) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.store[a.UserID] = a
	return nil
...
func (m *Store) Get(_ *http.Request, userID string) (loginflow.Attempt, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	a, ok := m.store[userID]
	if !ok {
