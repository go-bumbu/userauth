// Package loginflow is a composable login engine: a login attempt accumulates
// verified factors (password, TOTP, email code, …) until a Policy says the
// requirements are met, and only then is a session created.
//
// The package generalizes the fixed flows in handlers/login. It owns the
// security invariants that callers tend to get wrong:
//
...
//
// This package is experimental and not yet wired into handlers/login.
package loginflow
...
func (f *Flow) check() error {
	if f.Users == nil || f.Policy == nil || f.Session == nil {
		return errors.New("loginflow: Users, Policy and Session are required")
	}
	return nil
}
...
func (f *Flow) loadAttempt(r *http.Request, userID string) Attempt {
	...
	if err != nil {
		f.logger().Debug("loginflow: no usable attempt, starting fresh", "userID", userID, "reason", err)
	}
	return Attempt{UserID: userID, ExpiresAt: time.Now().Add(f.expiry())}
}
...
func (f *Flow) getEnabledUser(userID string) (userauth.User, bool, error) {
	...
	if err != nil {
		if errors.Is(err, userauth.ErrUserNotFound) {
			f.logger().Debug("loginflow: unknown user", "userID", userID)
			return userauth.User{}, false, nil
		}
		return userauth.User{}, false, err
	}
	if !user.Enabled {
		f.logger().Debug("loginflow: user disabled", "userID", userID)
		return userauth.User{}, false, nil
	}
...
func (f *Flow) Submit(r *http.Request, w http.ResponseWriter, userID, methodID, input string, keepLoggedIn bool) (Result, error) {
	...
	m := f.method(methodID)
	if m == nil {
		return Result{}, fmt.Errorf("loginflow: method %q not registered", methodID)
	}
	...
	if !contains(next, methodID) {
		f.logger().Debug("loginflow: method not offered", "userID", user.Id, "method", methodID, "satisfied", att.Satisfied)
		return Result{}, nil
	}

	ok, err = m.Verify(user.Id, input)
	if err != nil {
		return Result{}, fmt.Errorf("loginflow: verify %s: %w", methodID, err)
	}
	if !ok {
		f.logger().Debug("loginflow: factor verification failed", "userID", user.Id, "method", methodID)
		return Result{}, nil
	}
	...
	if done {
		if err := f.Session.LoginUser(r, w, user.Id, att.SessionKeepLoggedIn); err != nil {
			return Result{}, fmt.Errorf("loginflow: create session: %w", err)
		}
		if f.Attempts != nil {
			if err := f.Attempts.Clear(r, w, user.Id); err != nil {
				f.logger().Error("loginflow: failed to clear attempt", "userID", user.Id, "error", err)
			}
		}
		f.logger().Debug("loginflow: login complete", "userID", user.Id, "satisfied", att.Satisfied)
		return Result{OK: true, Done: true}, nil
	}

	if f.Attempts == nil {
		return Result{}, errors.New("loginflow: multi-step policy requires an Attempts store")
	}
	if err := f.Attempts.Set(r, w, att); err != nil {
		return Result{}, fmt.Errorf("loginflow: store attempt: %w", err)
	}
...
func (f *Flow) Initiate(r *http.Request, userID, methodID string) error {
	...
	m := f.method(methodID)
	if m == nil {
		return fmt.Errorf("loginflow: method %q not registered", methodID)
	}
	init, isInit := m.(Initiator)
	if !isInit {
		return fmt.Errorf("loginflow: method %q does not support initiation", methodID)
	}
	...
	if !contains(next, methodID) {
		f.logger().Debug("loginflow: initiation for method not offered", "userID", user.Id, "method", methodID)
		return nil
	}

	if err := init.Initiate(r.Context(), user); err != nil {
		f.logger().Error("loginflow: initiation failed", "userID", user.Id, "method", methodID, "error", err)
	}
	return nil
}
