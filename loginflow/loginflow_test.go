...
func newFixture(policy loginflow.Policy) *fixture {
	...
	flow := &loginflow.Flow{
		...
		Methods: []loginflow.Method{
			...
			loginflow.EmailCodeMethod(codes, deliverer),
		},
		Policy:   policy,
		Attempts: memory.New(),
		Session:  session,
...
func submit(t *testing.T, f *fixture, userID, method, input string) loginflow.Result {
	t.Helper()
...
func TestFlowSingleFactor(t *testing.T) {
	f := newFixture(loginflow.RequireAny(loginflow.Chain{"password"}))
...
func TestFlowPasswordPlusTOTP(t *testing.T) {
	policy := loginflow.RequireAny(loginflow.Chain{"password", "totp"})
...
func TestFlowEmailPlusTOTP(t *testing.T) {
	...
	policy := loginflow.RequireAny(loginflow.Chain{"email", "totp"})
	f := newFixture(policy)
...
func TestFlowAlternativeChains(t *testing.T) {
	...
	policy := loginflow.RequireAny(loginflow.Chain{"password", "totp"}, loginflow.Chain{"email"})
...
func TestFlowInitiate(t *testing.T) {
	policy := loginflow.RequireAny(loginflow.Chain{"email"})
